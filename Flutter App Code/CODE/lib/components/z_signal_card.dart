...
class _ZSignalCardState extends State<ZSignalCard> {
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Container(
                      decoration: BoxDecoration(color: widget.signal.entryType == 'Long' ? appColorGreen : appColorRed, borderRadius: BorderRadius.circular(5)),
                      padding: EdgeInsets.symmetric(horizontal: 4, vertical: 5),
                      margin: EdgeInsets.only(bottom: 3),
                      child: Text('${widget.signal.entryType.toUpperCase()} !!', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 12, color: Colors.white)),
                    ),
                    SizedBox(width: 6),
                    Text(
                ...
                if (widget.signal.comment != '')
                  Column(
                    children: [SizedBox(height: 8), Text('Comment: ${widget.signal.comment}')],
                  ),
                SizedBox(height: 8),
                Row(
                  children: [
                      ...
                      Expanded(
                        child: ZCard(
                          ...
                          child: Row(
                            ...
                            crossAxisAlignment: CrossAxisAlignment.center,
                            children: [
                              Text('View Analysis', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, height: 1.5)),
                              SizedBox(width: 4),
  ...
  _buildTargetCurrentPrice(bool isExpanded) {
    ...
    final entryVsCurrentPrice = widget.signal.compareEntryPriceWithCurrentPrice(price: appControlsProvider.getWSSymbolPrice(widget.signal), isPips: isForex);
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Text(hasApiAccess ? "Current Price" : 'Targets', style: TextStyle()),
          ),
          if (hasApiAccess) Text(appControlsProvider.getWSSymbolPrice(widget.signal).toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900)),
          Spacer(),
          if (hasApiAccess)
            Text(
              getPipsOrPercentStr(
              ...
              style: TextStyle(color: entryVsCurrentPrice < 0 ? appColorRed : appColorGreen, fontSize: 14, fontWeight: FontWeight.bold),
            ),
          SizedBox(width: 4),
          Icon(isExpanded ? Icons.arrow_drop_down : Icons.arrow_drop_up, size: 25),
        ],
      ),
    );
  }
  ...
  String getStatusText(Signal signal) {
    if (signal.takeProfit3Hit) return 'Bagged full profit  ${ZFormat.dateFormatSignal(signal.takeProfit3DateTime)}';
    if (signal.takeProfit2Hit) return 'Bagged target 2 profit  ${ZFormat.dateFormatSignal(signal.takeProfit2DateTime)}';
    if (signal.takeProfit1Hit) return 'Bagged target 1 profit  ${ZFormat.dateFormatSignal(signal.takeProfit1DateTime)}';
    if (signal.stopLossHit) return 'Stopped out @ ${ZFormat.dateFormatSignal(signal.takeProfit1DateTime)}';
    return 'In progress';
  }
  ...
  _buildStatusContainer({required String text, required bool isProfit, bool isInProgress = false}) {
    ...
    if (isInProgress)
      return Container(
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(6), color: isLightTheme ? Colors.grey.shade200 : Colors.white),
        padding: EdgeInsets.symmetric(horizontal: 4, vertical: 4),
        child: Row(
          children: [
            Text(text, style: TextStyle(color: Colors.black54, fontWeight: FontWeight.bold, fontSize: 13, height: 1.35)),
            SizedBox(width: 4),
            Icon(isProfit ? Icons.incomplete_circle_outlined : Icons.close, color: Colors.black54, size: 12),
          ],
        ),
      );
    return Container(
      ...
      child: Row(
        children: [
          Text(text, style: TextStyle(color: isProfit ? Colors.white70 : Color(0xFFFF637E), fontWeight: FontWeight.bold, fontSize: 13, height: 1.35)),
          SizedBox(width: 4),
          Icon(isProfit ? Icons.check : Icons.close, color: isProfit ? Colors.white70 : Color(0xFFFF637E), size: 12),
        ],
      ),
    );
  }
